...
with st.sidebar:
    ...
    side=option_menu(menu_title="Select to View",options=["Home",'Dataset','Visualization','Trends Over Years','Insights'],icons=['house','table','bar-chart','graph-up-arrow','search'])
...
# (The 'margin-bottom' in the CSS automatically handles the gap now!)
    ...
    t1,t2,t3,t4,t5=st.tabs(['Movies/TV Shows','Country Analysis','Genre','Ratings','Duration'])
    with t1:
        st.header("MOVIES VS TV SHOWS")
        c1,c2=st.columns(2)
        with c2:
            # st.subheader("Content Released each year")
            min_year=int(df['release_year'].min())
            max_year=int(df['release_year'].max())
            selected_year=st.slider("Select Release Year",1943,max_year)
